...
class CKYCMockProvider(IntegrationProvider):
    ...
    def verify(self, payload: dict) -> IntegrationResult:
        ...
        # 99.9% success rate in mock — effectively always passes for demo
        if rng.random() > 0.001:
...
# ── UAE Emirates ID Mock ───────────────────────────────────────────────────────
@register
class EmiratesIDMockProvider(IntegrationProvider):
    """Mock UAE Emirates ID verification. Real: ICP/PASS API."""
    provider_code    = "EMIRATES_ID_MOCK"
    integration_type = "IDENTITY"
    is_mock          = True

    def verify(self, payload: dict) -> IntegrationResult:
        rng  = _seed(payload.get("applicant_ref", ""), "emirates")
        name = payload.get("full_name") or "APPLICANT"
        age  = payload.get("age", 35)
        dob_year = date.today().year - int(age)
        dob  = date(dob_year, rng.randint(1,12), rng.randint(1,28))
        eid  = f"784-{dob_year}-{rng.randint(1000000,9999999)}-{rng.randint(1,9)}"
        return IntegrationResult(
            success=True, provider_code=self.provider_code,
            integration_type=self.integration_type,
            kyc_verified=True, kyc_name=name.upper(), kyc_dob=str(dob),
            kyc_address=f"Villa {rng.randint(1,99)}, Al Barsha, Dubai, UAE",
            kyc_pan=eid,
            confidence_score=round(rng.uniform(0.93,0.99),2),
            raw_response={"emirates_id":eid,"verified":True,"source":"EMIRATES_ID_MOCK"},
            notes="Emirates ID mock — use UAE PASS live API for production",
            expires_in_days=365,
        )


# ── UAE AECB Credit Mock ───────────────────────────────────────────────────────
@register
class AECBMockProvider(IntegrationProvider):
    """Mock UAE Al Etihad Credit Bureau. Score 300-900."""
    provider_code    = "AECB_MOCK"
    integration_type = "CREDIT"
    is_mock          = True

    def verify(self, payload: dict) -> IntegrationResult:
        rng   = _seed(payload.get("applicant_ref", ""), "aecb")
        score = min(900, max(300, 660 + rng.randint(-80, 100)))
        flags = []
        if score < 600: flags.append("HIGH_RISK_AECB")
        if rng.random() < 0.05: flags.append("LATE_PAYMENT_UAE")
        return IntegrationResult(
            success=True, provider_code=self.provider_code,
            integration_type=self.integration_type,
            credit_score=score, credit_bureau="Al Etihad Credit Bureau (Mock)",
            credit_report_ref=f"AECB{rng.randint(10000000,99999999)}",
            credit_flags=flags,
            confidence_score=round(rng.uniform(0.90,0.99),2),
            raw_response={"score":score,"bureau":"AECB","country":"AE"},
            notes="AECB mock — use live AECB API for production", expires_in_days=30,
        )


# ── Singapore MyInfo Mock ─────────────────────────────────────────────────────
@register
class MyInfoMockProvider(IntegrationProvider):
    """Mock Singapore MyInfo/SingPass. Real: NDI MyInfo API."""
    provider_code    = "MYINFO_MOCK"
    integration_type = "IDENTITY"
    is_mock          = True

    def verify(self, payload: dict) -> IntegrationResult:
        rng  = _seed(payload.get("applicant_ref", ""), "myinfo")
        name = payload.get("full_name") or "APPLICANT"
        age  = payload.get("age", 35)
        dob_year = date.today().year - int(age)
        nric = f"S{dob_year % 100:02d}{rng.randint(10000,99999)}{rng.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZ')}"
        return IntegrationResult(
            success=True, provider_code=self.provider_code,
            integration_type=self.integration_type,
            kyc_verified=True, kyc_name=name.upper(),
            kyc_dob=str(date(dob_year, rng.randint(1,12), rng.randint(1,28))),
            kyc_pan=nric,
            kyc_address=f"Blk {rng.randint(1,999)} Ang Mo Kio Ave {rng.randint(1,10)}, Singapore {rng.randint(100000,999999)}",
            confidence_score=round(rng.uniform(0.95,0.99),2),
            raw_response={"nric":nric,"verified":True,"source":"MYINFO_MOCK","nationality":"SINGAPOREAN"},
            notes="MyInfo mock — use SingPass/NDI MyInfo API for production",
            expires_in_days=365,
        )


# ── Singapore CBS Credit Mock ─────────────────────────────────────────────────
@register
class CBSMockProvider(IntegrationProvider):
    """Mock Singapore Credit Bureau. Score 1000-2000."""
    provider_code    = "CBS_MOCK"
    integration_type = "CREDIT"
    is_mock          = True

    def verify(self, payload: dict) -> IntegrationResult:
        rng   = _seed(payload.get("applicant_ref", ""), "cbs")
        score = min(2000, max(1000, 1600 + rng.randint(-200, 200)))
        flags = []
        if score < 1300: flags.append("POOR_CBS_SCORE")
        if rng.random() < 0.05: flags.append("LATE_PAYMENT_SG")
        return IntegrationResult(
            success=True, provider_code=self.provider_code,
            integration_type=self.integration_type,
            credit_score=score, credit_bureau="Credit Bureau Singapore (Mock)",
            credit_report_ref=f"CBS{rng.randint(10000000,99999999)}",
            credit_flags=flags,
            confidence_score=round(rng.uniform(0.90,0.99),2),
            raw_response={"score":score,"bureau":"CBS","country":"SG","risk_grade":"AA" if score>1800 else "BB"},
            notes="CBS mock — use live CBS Singapore API for production", expires_in_days=30,
        )


# ── UK Yoti Identity Mock ─────────────────────────────────────────────────────
@register
class YotiMockProvider(IntegrationProvider):
    """Mock UK Yoti digital identity. Real: Yoti API."""
    provider_code    = "YOTI_MOCK"
    integration_type = "IDENTITY"
    is_mock          = True

    def verify(self, payload: dict) -> IntegrationResult:
        rng  = _seed(payload.get("applicant_ref", ""), "yoti")
        name = payload.get("full_name") or "APPLICANT"
        age  = payload.get("age", 35)
        dob_year = date.today().year - int(age)
        return IntegrationResult(
            ...
            kyc_verified=True, kyc_name=name.upper(),
            kyc_dob=str(date(dob_year, rng.randint(1,12), rng.randint(1,28))),
            kyc_address=f"{rng.randint(1,200)} High Street, London, {rng.choice(['SW1A 1AA','EC1A 1BB','W1A 1AA'])}",
            confidence_score=round(rng.uniform(0.92,0.99),2),
            raw_response={"yoti_id":f"YOTI{rng.randint(100000,999999)}","verified":True,"source":"YOTI_MOCK","country":"GB"},
            notes="Yoti mock — use live Yoti API for production",
            expires_in_days=365,
        )


# ── UK Experian Credit Mock ────────────────────────────────────────────────────
@register
class ExperianUKMockProvider(IntegrationProvider):
    """Mock UK Experian credit score. Score 0-999."""
    provider_code    = "EXPERIAN_UK_MOCK"
    integration_type = "CREDIT"
    is_mock          = True

    def verify(self, payload: dict) -> IntegrationResult:
        rng   = _seed(payload.get("applicant_ref", ""), "experian_uk")
        score = min(999, max(0, 720 + rng.randint(-150, 150)))
        flags = []
        if score < 560: flags.append("POOR_UK_CREDIT")
        if rng.random() < 0.04: flags.append("CCJ_REGISTERED")
        return IntegrationResult(
            success=True, provider_code=self.provider_code,
            integration_type=self.integration_type,
            credit_score=score, credit_bureau="Experian UK (Mock)",
            credit_report_ref=f"EXUK{rng.randint(10000000,99999999)}",
            credit_flags=flags,
            confidence_score=round(rng.uniform(0.90,0.99),2),
            raw_response={"score":score,"bureau":"Experian UK","country":"GB"},
            notes="Experian UK mock — use live Experian API for production", expires_in_days=30,
        )


# ── US Socure Identity Mock ────────────────────────────────────────────────────
@register
class SocureMockProvider(IntegrationProvider):
    """Mock US Socure digital identity. Real: Socure API."""
    provider_code    = "SOCURE_MOCK"
    integration_type = "IDENTITY"
    is_mock          = True

    def verify(self, payload: dict) -> IntegrationResult:
        rng  = _seed(payload.get("applicant_ref", ""), "socure")
        name = payload.get("full_name") or "APPLICANT"
        age  = payload.get("age", 35)
        dob_year = date.today().year - int(age)
        ssn_masked = f"XXX-XX-{rng.randint(1000,9999)}"
        return IntegrationResult(
            success=True, provider_code=self.provider_code,
            integration_type=self.integration_type,
            kyc_verified=True, kyc_name=name.upper(),
            kyc_dob=str(date(dob_year, rng.randint(1,12), rng.randint(1,28))),
            kyc_pan=ssn_masked,
            kyc_address=f"{rng.randint(100,9999)} Main St, New York, NY {rng.randint(10000,99999)}",
            confidence_score=round(rng.uniform(0.93,0.99),2),
            raw_response={"socure_id":f"SC{rng.randint(100000,999999)}","verified":True,"source":"SOCURE_MOCK","country":"US"},
            notes="Socure mock — use live Socure API for production",
            expires_in_days=365,
        )


# ── US Equifax Credit Mock ─────────────────────────────────────────────────────
@register
class EquifaxMockProvider(IntegrationProvider):
    """Mock US Equifax credit score. FICO range 300-850."""
    provider_code    = "EQUIFAX_MOCK"
    integration_type = "CREDIT"
    is_mock          = True

    def verify(self, payload: dict) -> IntegrationResult:
        rng   = _seed(payload.get("applicant_ref", ""), "equifax")
        score = min(850, max(300, 700 + rng.randint(-120, 100)))
        flags = []
        if score < 580: flags.append("POOR_FICO_SCORE")
        if rng.random() < 0.03: flags.append("BANKRUPTCY_HISTORY")
        return IntegrationResult(
            success=True, provider_code=self.provider_code,
            integration_type=self.integration_type,
            credit_score=score, credit_bureau="Equifax (Mock)",
            credit_report_ref=f"EQ{rng.randint(10000000,99999999)}",
            credit_flags=flags,
            confidence_score=round(rng.uniform(0.90,0.99),2),
            raw_response={"score":score,"bureau":"Equifax","country":"US","score_model":"FICO8"},
            notes="Equifax mock — use live Equifax API for production", expires_in_days=30,
        )
@register
