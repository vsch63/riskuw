...
class StepIn(BaseModel):
    ...
    factor:         float = 1.0
    parameter_type: str
    user_value:     Optional[float] = None  # for USER_VALUE (true constant)
    user_label:     Optional[str]   = None  # for USER_LABEL (per-proposal named value)
...
def add_step(product_code: str, formula_id: str, body: StepIn, user: CurrentUser = CurrentUser):
    ...
    try:
        conn.autocommit = False
        cur = conn.cursor()
        # Verify formula belongs to product
        cur.execute(
