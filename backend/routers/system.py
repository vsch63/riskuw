...
def list_states(
    ...
    try:
        ...
        sql = "SELECT id, country_code, state_code, state_name, is_active FROM state_codes"
        if where:
            sql += " WHERE " + " AND ".join(where)
        sql += " ORDER BY country_code, state_name"
...
def create_state(body: dict, current: CurrentUser):
    conn, release = _get_db()
    try:
        cur = conn.cursor()
        cur.execute("""
            ...
            RETURNING id
        """, (
            body.get("country_code","IN").upper(),
            body.get("state_code","").upper(),
            body.get("state_name",""),
        ...
        row = cur.fetchone()
        cur.close()
        return {"id": _row(row).get("id") if row else None, "message": "Saved"}
    except Exception as e:
        conn.rollback()
...
def update_state(state_id: int, body: dict, current: CurrentUser):
    conn, release = _get_db()
    try:
        cur = conn.cursor()
        cur.execute(
            "UPDATE state_codes SET state_name=%s, is_active=%s WHERE id=%s",
            (body.get("state_name"), body.get("is_active", True), state_id)
        )
        conn.commit()
        cur.close()
...
def delete_state(state_id: int, current: CurrentUser):
    conn, release = _get_db()
    try:
        cur = conn.cursor()
        cur.execute("DELETE FROM state_codes WHERE id=%s", (state_id,))
        conn.commit()
        cur.close()
...
@router.post("/states/load-preset")
def load_state_preset(body: dict, current: CurrentUser):
    country = body.get("country_code", "IN").upper()
    PRESETS = {
        "IN": [
    ...
    states = PRESETS.get(country, [])
    if not states:
        raise HTTPException(400, f"No preset available for {country}")
    conn, release = _get_db()
    try:
        cur = conn.cursor()
        inserted = 0
        ...
        conn.commit()
        cur.close()
        return {"message": f"Loaded {inserted} state codes for {country}"}
    finally:
        release(conn)
...
def list_error_codes(current: CurrentUser):
    ...
    try:
        cur = conn.cursor()
        cur.execute("""
            SELECT id, error_number, error_code AS code,
                   category, severity, description AS message,
                   resolution_hint AS resolution, is_active, created_at
            FROM error_codes ORDER BY error_number, error_code
        """)
        rows = cur.fetchall()
        cur.close()
        return [_row(r) for r in rows]
...
def create_error_code(body: ErrorCodeCreate, current: CurrentUser):
    ...
    try:
        ...
        cur.execute(
            """
            INSERT INTO error_codes (error_code, category, severity, description, resolution_hint, is_active)
