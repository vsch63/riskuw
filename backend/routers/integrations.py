...
def list_providers(current: CurrentUser, integration_type: str = "ALL"):
    ...
    try:
        ...
        cur.execute(f"""
            SELECT id, provider_code, provider_name, integration_type,
                   is_enabled, is_mock, api_endpoint,
                   config_json, created_at, updated_at
            FROM integration_config
            {where}
            ORDER BY integration_type, provider_name
        """, params)
...
def run_verification(body: VerifyRequest, current: CurrentUser):
    ...
    try:
        ...
        # Find provider
        where = "tenant_id=%s AND integration_type=%s AND is_enabled=true"
        params: list = [current.tenant_id, body.integration_type]
        ...
        cur.execute("""
            INSERT INTO integration_requests (
                ...
                provider_code, status, request_payload, requested_by
            ) VALUES (%s,%s,%s,%s,%s,'PENDING',%s,%s)
            RETURNING id, request_ref
        """, (
            current.tenant_id, body.case_ref_id, body.applicant_ref,
            body.integration_type, cfg["provider_code"],
